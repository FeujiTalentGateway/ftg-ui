...
export class DetailedUserResultComponent implements OnInit {
  ...
  ngOnInit() {
    this.examCode = this.activatedRoute.snapshot.paramMap.get(
      'examCode'
    ) as string;
    this.userId = this.activatedRoute.snapshot.paramMap.get('userId') as
      | number
      | null
      | undefined;
    this.examObject$ = this.examService.getExamByCode(this.examCode);
    this.detailedUserResultObject$ = this.examService.getDetailedUserResult(
      this.examCode,
  ...
  calculateProgress(): number {
    ...
    const userDurationInSeconds = this.parseDurationToSeconds(
      this.examDuration
    );
    const progressPercentage =
      (userDurationInSeconds / examDurationInSeconds) * 100;
